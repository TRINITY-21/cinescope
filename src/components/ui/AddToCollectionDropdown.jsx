...
export default function AddToCollectionDropdown({
  ...
  return (
    <div className="relative" ref={ref}>
      {iconOnly ? (
        <IconButton aria-label="Add to collection" title="Add to collection" onClick={() => setOpen((o) => !o)}>
          {collectionIcon}
        </IconButton>
      ) : (
        <Button
                ...
                onClick={() => {
                  if (inCol) {
                    ...
                    toast({
                      ...
                      action: {
                        label: 'Undo',
                        onClick: () => addToCollection(c.id, item),
                      },
                    });
                  } else {
                    addToCollection(c.id, item);
                    toast({ message: `Added to ${c.name}`, variant: 'success' });
                  }
                  setOpen(false);
                }}
              >
                ...
                {inCol && <span className="text-accent-peach">✓</span>}
              </button>
            );
          })}
        </div>
