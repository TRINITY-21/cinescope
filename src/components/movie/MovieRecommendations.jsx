...
export default function MovieRecommendations({ recommendations, sourceTitle }) {
  const sorted = useMemo(() => sortByRatingThenYear(recommendations || []), [recommendations]);
  if (sorted.length === 0) return null;

  const seeAllHref = sourceTitle ? `/like/${slugify(sourceTitle)}` : null;
  const heading = sourceTitle ? `Movies like ${sourceTitle}` : 'Recommended For You';
  ...
  return (
    <div>
      <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-5">
        ...
        <h3 className="text-base sm:text-lg font-semibold text-white">{heading}</h3>
        <span className="text-[10px] sm:text-xs text-text-muted">{sorted.length} movies</span>
        {seeAllHref && (
          <Link
            to={seeAllHref}
            className="ml-auto inline-flex items-center gap-1 text-xs sm:text-sm font-semibold text-accent-peach hover:text-accent-gold transition-colors"
          >
            See all
