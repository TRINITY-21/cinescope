    ...
    <a href={link} target="_blank" rel="noopener noreferrer" className="group">
      ...
      >
        ...
        <div className="min-w-0">
          <p className="text-xs sm:text-sm font-medium text-white break-words min-w-0 group-hover:text-accent-peach transition-colors">{provider.provider_name}</p>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-2xl p-4 sm:p-6">
      ...
      {rent.length > 0 && (
        <div className="mb-3 sm:mb-5">
          <p className="text-xs text-text-muted uppercase tracking-wider mb-2 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-accent-peach" />Rent
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
