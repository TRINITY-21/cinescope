...
import { findShowByImdb, searchTmdbShow, getShowImages, getShowVideos, TMDB_IMAGE_BASE, hasTmdbKey } from '../../api/tmdb';
    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-center w-full h-full p-4 md:p-8" onClick={(e) => e.stopPropagation()}>
        ...
        <motion.img
          ...
          src={src}
          alt=""
          className={`${type === 'poster' ? 'max-h-[85vh] w-auto' : 'max-w-[90vw] max-h-[85vh] w-auto'} object-contain rounded-lg shadow-2xl`}
        />
        ...
        {/* Counter */}
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 px-4 py-1.5 rounded-full bg-black/60 text-xs text-text-secondary font-mono">
          {index + 1} / {images.length}
        </div>

        {/* Resolution */}
        <div className="absolute top-4 left-4 z-20 px-3 py-1 rounded-lg bg-black/60 text-[11px] text-text-muted font-mono">
...
export default function MediaSection({ showName, showYear, imdbId, selectVideosTrigger = 0 }) {
    ...
    async function load() {
      ...
      try {
        ...
        const [imgData, vidData] = await Promise.allSettled([
          getShowImages(tmdbShow.id),
          getShowVideos(tmdbShow.id),
        ]);

        if (imgData.status === 'fulfilled' && imgData.value) setImages(imgData.value);
        if (vidData.status === 'fulfilled') setVideos(vidData.value.filter((v) => v.site === 'YouTube'));
      } catch (err) {
  ...
  useEffect(() => {
    if (selectVideosTrigger > 0) setActiveTab('videos');
  }, [selectVideosTrigger]);
      ...
      <div className="glass rounded-2xl p-4 sm:p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {Array.from({ length: 4 }, (_, i) => (
            <div key={i} className="aspect-video bg-bg-elevated rounded-xl animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-1 mb-3 sm:mb-5 p-1 rounded-xl bg-white/[0.03] w-fit max-w-full overflow-x-auto hide-scrollbar">
        {visibleTabs.map((tab) => (
          <button
            ...
            className={`relative px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-all whitespace-nowrap ${
              activeTab === tab
                ? 'text-white'
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {displayVideos.map((video, i) => (
                <motion.button
                  ...
                  className="group relative aspect-video rounded-xl overflow-hidden ring-1 ring-white/[0.06] hover:ring-accent-red/30 transition-all text-left"
                >
                  <img
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-br from-accent-red to-accent-red/80 flex items-center justify-center shadow-[0_0_30px_rgba(196,85,58,0.5),0_0_60px_rgba(196,85,58,0.2)] group-hover:shadow-[0_0_40px_rgba(196,85,58,0.7),0_0_80px_rgba(196,85,58,0.3)] group-hover:scale-110 transition-all duration-300 border border-white/20">
                      <svg width="28" height="28" viewBox="0 0 24 24" fill="white" className="ml-1 drop-shadow-lg">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {video.official && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-accent-red/20 text-accent-red">Official</span>
                      )}
                    </div>
                  </div>
                </motion.button>
      ...
      {/* Lightbox — portaled to body so fixed positioning works */}
      {createPortal(
        <AnimatePresence>
          {lightbox && (
            <Lightbox
      ...
      {/* Video Modal — portaled to body so it centers on viewport */}
      {createPortal(
        <AnimatePresence>
          {videoModal && (
