  ...
  return (
    <GlassPanel glow="violet" className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="text-center">
          <p className="text-lg sm:text-2xl font-bold text-accent-peach">{calc.progress.percentage}%</p>
      ...
      {calc.progress.watched > 0 && (
        <div>
          <div className="w-full h-2 bg-bg-primary rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-accent-peach to-accent-red rounded-full transition-all duration-700" style={{ width: `${calc.progress.percentage}%` }} />
          </div>
        ...
        <div className="border-t border-white/5 pt-4 space-y-4">
          <div className="flex rounded-lg overflow-hidden border border-white/10 w-fit">
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium transition-colors ${mode === 'hours' ? 'bg-accent-peach/80 text-white' : 'bg-white/5 text-text-muted hover:text-white'}`}
            >
              I have time
            </button>
            <button
              onClick={() => setMode('date')}
              className={`px-3 py-1.5 text-xs font-medium transition-colors ${mode === 'date' ? 'bg-accent-peach/80 text-white' : 'bg-white/5 text-text-muted hover:text-white'}`}
            >
              Finish by date
            </button>
          </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full accent-accent-peach h-1.5 rounded-full appearance-none bg-white/10 cursor-pointer"
                />
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-bg-primary/50 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-accent-peach/50"
                />
              </div>
              {targetDate && (
                <div className="bg-bg-primary/50 rounded-xl p-4">
                  <div className="grid grid-cols-3 gap-3 text-center">
                    ...
                    <div>
                      <p className={`text-xl font-bold ${targetHoursPerDay > 8 ? 'text-red-400' : 'text-accent-peach'}`}>{targetHoursPerDay}h</p>
